const getCine = async () => {
    ...
    const [data, data2, data3] = await Promise.all([
        /*fetch(`https://oaemdl.es/cinestar_sweb_php/cines/${idd}`),
        fetch(`https://oaemdl.es/cinestar_sweb_php/cines/${idd}/tarifas`),
        fetch(`https://oaemdl.es/cinestar_sweb_php/cines/${idd}/peliculas`)*/
        fetch(`https://localhost/cinestar_sweb_php/cines/${idd}`),
        fetch(`https://localhost/cinestar_sweb_php/cines/${idd}/tarifas`),
        fetch(`https://localhost/cinestar_sweb_php/cines/${idd}/peliculas`)
    ]);

    if (data.status === 200 && data2.status === 200 && data3.status === 200) {
        const [cine, tarifa, pelicula] = await Promise.all([
            data.json(),
            data2.json(),
            data3.json()
        ]);
