const getCine = async () => {
  ...
  const data1 = await fetch(`https://oaemdl.es/cinestar_sweb_php/cines/${idd}/tarifas`);
  const data2 = await fetch(`https://oaemdl.es/cinestar_sweb_php/cines/${idd}/peliculas`);

  if (data.status == 200 && data1.status == 200 && data2.status == 200) {

    const cine = await data.json();
    const tarifa = await data1.json();
    const pelicula = await data2.json();
